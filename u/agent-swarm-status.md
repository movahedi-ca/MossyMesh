...
| Box | Evidence | Status |
|-----|----------|--------|
| P4-WASM | `engine` still rlib-oriented wasip1 notes; no cdylib `.wasm` load path proven in sandbox; host sim accepts `\0asm` stubs only | **OPEN** — 05 + 14 + 16 |
| P4-MSG | untracked/WIP `mesh-transport/src/messaging.rs` (JobMeshBus distribute/collect + unit tests) may exist in shared worktree; **not on origin/main or origin/agent/02**; lib.rs not re-exporting on main | **OPEN** — 02 must commit+push; 03 route; 16 smoke |
...
| **PUSH** | (all branches) | **continuous push** every ~60s — never lose WIP | shared worktree thrash risk — prefer worktrees |
