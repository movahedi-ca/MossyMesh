...
//! production. Production nodes should plug in a verifier that re-checks a
//! real MinRoot proof (or a transport-side pre-verified receipt) via
//! [`VdfVerifier`].
...
pub struct VdfReceipt {
    ...
    pub final_x: u64,
    /// Parameter / curve id (stub uses a free `u32` tag).
    pub modulus_id: u32,
    /// Job metadata bound into DID mint: `DID = H(final_x || job_meta)`.
    pub job_meta: Vec<u8>,
...
///
/// Production: wrap transport MinRoot verify. Tests: use
/// [`DomainSeparatedHashVdfStub`].
pub trait VdfVerifier {
    /// Return `true` when `receipt` proves the claimed sequential delay and
...
// --- internal hashing helpers (no extra crate deps) -------------------------
